...
def _apply_status(win, ok: bool):
    # 1) Si existe un label de estado, úsalo
    ...
    # 2) Luz (●) en la esquina superior derecha
    ...
    if light is None:
        ...
        win.resizeEvent = (lambda ev, _old=getattr(win,"resizeEvent",None): (_old(ev) if callable(_old) else None, _reposition()))[1]
...
def attach_status(win, interval_ms: int = 15000):
    # aplica inmediatamente y luego cada N ms
    ok = _is_connected()
